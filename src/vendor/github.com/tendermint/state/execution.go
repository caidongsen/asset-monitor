...
func (s *State) ExecCommitBlock(appConnConsensus proxy.AppConnConsensus, block *types.Block, logger log.Logger) ([]byte, error) {
	var eventCache types.Fireable // nil
	abciResponses, err := execBlockOnProxyApp(eventCache, appConnConsensus, block, logger)
	if err != nil {
		logger.Error("Error executing block on proxy app", "height", block.Height, "err", err)
		return nil, err
	}

	// IMPORTANT: make TxIndexer work even during replayBlocks
	// index txs. This could run in the background
	s.indexTxs(abciResponses)
