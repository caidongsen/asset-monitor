	...
	for {
		...
		if (0 < prs.Height) && (prs.Height < rs.Height) {
			...
			if index, ok := prs.ProposalBlockParts.Not().PickRandom(); ok {
				...
				if blockMeta == nil {
					logger.Error("Failed to load block meta", "peerHeight", prs.Height, "ourHeight", rs.Height, "blockstoreHeight", conR.conS.blockStore.Height(), "pv", conR.conS.privValidator)
