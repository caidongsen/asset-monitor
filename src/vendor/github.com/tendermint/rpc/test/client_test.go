...
func testBlock(t *testing.T, client rpc.HTTPClient, height int) *ctypes.ResultBlock {
	require := require.New(t)

	result := new(ctypes.ResultBlock)
	_, err := client.Call("block", map[string]interface{}{"height": height}, result)
	require.Nil(err)
...
func TestWSNewBlockAndTxResult(t *testing.T) {
	wsc := GetWSClient()
	eid := types.EventStringNewBlock()
	require.Nil(t, wsc.Subscribe(eid))
	...
	waitForEvent(t, wsc, eid, true, func() {}, func(eid string, eventData interface{}) error {
		...
		result := testBlock(t, GetJSONClient(), block.Height)
		t.Log(result.BlockTxsResults)
		return nil
	})
}
