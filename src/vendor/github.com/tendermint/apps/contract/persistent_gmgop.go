...
type PersistentContractApplication struct {
	app *ContractApplication
	db  dbm.DB
...
func NewPersistentContractApplication(dbDir string) *PersistentContractApplication {
	db := dbm.NewDB("contract", "leveldb", dbDir)
	...
	return &PersistentContractApplication{
		app:    &ContractApplication{state: stateTree},
		db:     db,
		logger: log.NewNopLogger(),
	}
}

func (app *PersistentContractApplication) SetLogger(l log.Logger) {
	app.logger = l
}

func (app *PersistentContractApplication) Info() (resInfo types.ResponseInfo) {
...
func (app *PersistentContractApplication) SetOption(key string, value string) (log string) {
	return app.app.SetOption(key, value)
}

// tx is either "key=value" or just arbitrary bytes
func (app *PersistentContractApplication) DeliverTx(tx []byte) types.Result {
...
func (app *PersistentContractApplication) CheckTx(tx []byte) types.Result {
	return app.app.CheckTx(tx)
}

func (app *PersistentContractApplication) Commit() types.Result {
	// Save
...
func (app *PersistentContractApplication) Query(reqQuery types.RequestQuery) types.ResponseQuery {
	return app.app.Query(reqQuery)
}

// Save the validators in the merkle tree
func (app *PersistentContractApplication) InitChain(validators []*types.Validator) {
...
func (app *PersistentContractApplication) BeginBlock(hash []byte, header *types.Header) {
...
func (app *PersistentContractApplication) EndBlock(height uint64) (resEndBlock types.ResponseEndBlock) {
...
func (app *PersistentContractApplication) Validators() (validators []*types.Validator) {
...
func (app *PersistentContractApplication) execValidatorTx(tx []byte) types.Result {
...
func (app *PersistentContractApplication) updateValidator(v *types.Validator) types.Result {
