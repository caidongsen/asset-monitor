...
type PersistentGmgopApplication struct {
	app *GmgopApplication
	db  dbm.DB
...
func NewPersistentGmgopApplication(dbDir string) *PersistentGmgopApplication {
	db := dbm.NewDB("gmgop", "leveldb", dbDir)
	...
	return &PersistentGmgopApplication{
		app:    &GmgopApplication{state: stateTree},
		db:     db,
		logger: log.NewNopLogger(),
	}
}

func (app *PersistentGmgopApplication) SetLogger(l log.Logger) {
	app.logger = l
}

func (app *PersistentGmgopApplication) Info() (resInfo types.ResponseInfo) {
...
func (app *PersistentGmgopApplication) SetOption(key string, value string) (log string) {
	return app.app.SetOption(key, value)
}

// tx is either "key=value" or just arbitrary bytes
func (app *PersistentGmgopApplication) DeliverTx(tx []byte) types.Result {
...
func (app *PersistentGmgopApplication) CheckTx(tx []byte) types.Result {
	return app.app.CheckTx(tx)
}

func (app *PersistentGmgopApplication) Commit() types.Result {
	// Save
...
func (app *PersistentGmgopApplication) Query(reqQuery types.RequestQuery) types.ResponseQuery {
	return app.app.Query(reqQuery)
}

// Save the validators in the merkle tree
func (app *PersistentGmgopApplication) InitChain(validators []*types.Validator) {
...
func (app *PersistentGmgopApplication) BeginBlock(hash []byte, header *types.Header) {
...
func (app *PersistentGmgopApplication) EndBlock(height uint64) (resEndBlock types.ResponseEndBlock) {
...
func (app *PersistentGmgopApplication) Validators() (validators []*types.Validator) {
...
func (app *PersistentGmgopApplication) execValidatorTx(tx []byte) types.Result {
...
func (app *PersistentGmgopApplication) updateValidator(v *types.Validator) types.Result {
