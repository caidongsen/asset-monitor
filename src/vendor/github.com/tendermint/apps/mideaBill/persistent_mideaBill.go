...
type PersistentMideaBillApplication struct {
	app *MideaBillApplication
	db  dbm.DB
...
func NewPersistentMideaBillApplication(dbDir string) *PersistentMideaBillApplication {
	db := dbm.NewDB("mideaBill", "leveldb", dbDir)
	...
	return &PersistentMideaBillApplication{
		app:    &MideaBillApplication{state: stateTree},
		db:     db,
		logger: log.NewNopLogger(),
	}
}

func (app *PersistentMideaBillApplication) SetLogger(l log.Logger) {
	app.logger = l
}

func (app *PersistentMideaBillApplication) Info() (resInfo types.ResponseInfo) {
...
func (app *PersistentMideaBillApplication) SetOption(key string, value string) (log string) {
	return app.app.SetOption(key, value)
}

// tx is either "key=value" or just arbitrary bytes
func (app *PersistentMideaBillApplication) DeliverTx(tx []byte) types.Result {
...
func (app *PersistentMideaBillApplication) CheckTx(tx []byte) types.Result {
	if isValidatorTx(tx) {
		return types.OK
	}
...
func (app *PersistentMideaBillApplication) Commit() types.Result {
	// Save
...
func (app *PersistentMideaBillApplication) Query(reqQuery types.RequestQuery) types.ResponseQuery {
	return app.app.Query(reqQuery)
}

// Save the validators in the merkle tree
func (app *PersistentMideaBillApplication) InitChain(validators []*types.Validator) {
...
func (app *PersistentMideaBillApplication) BeginBlock(hash []byte, header *types.Header) {
...
func (app *PersistentMideaBillApplication) EndBlock(height uint64) (resEndBlock types.ResponseEndBlock) {
...
func (app *PersistentMideaBillApplication) Validators() (validators []*types.Validator) {
...
func (app *PersistentMideaBillApplication) execValidatorTx(tx []byte) types.Result {
...
func (app *PersistentMideaBillApplication) updateValidator(v *types.Validator) types.Result {
