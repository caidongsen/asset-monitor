...
type PersistentSmscApplication struct {
	app *SmscApplication
	db  dbm.DB
...
func NewPersistentSmscApplication(dbDir string) *PersistentSmscApplication {
	db := dbm.NewDB("smsc", "leveldb", dbDir)
	...
	return &PersistentSmscApplication{
		app:    &SmscApplication{state: stateTree},
		db:     db,
		logger: log.NewNopLogger(),
	}
}

func (app *PersistentSmscApplication) SetLogger(l log.Logger) {
	app.logger = l
}

func (app *PersistentSmscApplication) Info() (resInfo types.ResponseInfo) {
...
func (app *PersistentSmscApplication) SetOption(key string, value string) (log string) {
	return app.app.SetOption(key, value)
}

// tx is either "key=value" or just arbitrary bytes
func (app *PersistentSmscApplication) DeliverTx(tx []byte) types.Result {
...
func (app *PersistentSmscApplication) CheckTx(tx []byte) types.Result {
	return app.app.CheckTx(tx)
}

func (app *PersistentSmscApplication) Commit() types.Result {
	// Save
...
func (app *PersistentSmscApplication) Query(reqQuery types.RequestQuery) types.ResponseQuery {
	return app.app.Query(reqQuery)
}

// Save the validators in the merkle tree
func (app *PersistentSmscApplication) InitChain(validators []*types.Validator) {
...
func (app *PersistentSmscApplication) BeginBlock(hash []byte, header *types.Header) {
...
func (app *PersistentSmscApplication) EndBlock(height uint64) (resEndBlock types.ResponseEndBlock) {
...
func (app *PersistentSmscApplication) Validators() (validators []*types.Validator) {
...
func (app *PersistentSmscApplication) execValidatorTx(tx []byte) types.Result {
...
func (app *PersistentSmscApplication) updateValidator(v *types.Validator) types.Result {
