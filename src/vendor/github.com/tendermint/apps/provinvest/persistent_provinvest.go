...
type PersistentProvinvestApplication struct {
	app *ProvinvestApplication
	db  dbm.DB
...
func NewPersistentProvinvestApplication(dbDir string) *PersistentProvinvestApplication {
	...
	return &PersistentProvinvestApplication{
		app:    &ProvinvestApplication{state: stateTree},
		db:     db,
		logger: log.NewNopLogger(),
	}
}

func (app *PersistentProvinvestApplication) SetLogger(l log.Logger) {
	app.logger = l
}

func (app *PersistentProvinvestApplication) Info() (resInfo types.ResponseInfo) {
...
func (app *PersistentProvinvestApplication) SetOption(key string, value string) (log string) {
	return app.app.SetOption(key, value)
}

// tx is either "key=value" or just arbitrary bytes
func (app *PersistentProvinvestApplication) DeliverTx(tx []byte) types.Result {
...
func (app *PersistentProvinvestApplication) CheckTx(tx []byte) types.Result {
	return app.app.CheckTx(tx)
}

func (app *PersistentProvinvestApplication) Commit() types.Result {
	// Save
...
func (app *PersistentProvinvestApplication) Query(reqQuery types.RequestQuery) types.ResponseQuery {
	return app.app.Query(reqQuery)
}

// Save the validators in the merkle tree
func (app *PersistentProvinvestApplication) InitChain(validators []*types.Validator) {
...
func (app *PersistentProvinvestApplication) BeginBlock(hash []byte, header *types.Header) {
...
func (app *PersistentProvinvestApplication) EndBlock(height uint64) (resEndBlock types.ResponseEndBlock) {
...
func (app *PersistentProvinvestApplication) Validators() (validators []*types.Validator) {
...
func (app *PersistentProvinvestApplication) execValidatorTx(tx []byte) types.Result {
...
func (app *PersistentProvinvestApplication) updateValidator(v *types.Validator) types.Result {
